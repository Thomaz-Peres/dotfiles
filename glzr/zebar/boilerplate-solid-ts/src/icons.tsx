  ...
  switch (network.defaultInterface?.type) {
    ...
    case 'wifi':
      if (network.defaultGateway?.signalStrength >= 80) {
        ...
        network.defaultGateway?.signalStrength >= 65
      ) {
        return <i class="nf nf-md-wifi_strength_3"></i>;
      } else if (
        network.defaultGateway?.signalStrength >= 40
      ) {
        return <i class="nf nf-md-wifi_strength_2"></i>;
      } else if (
        network.defaultGateway?.signalStrength >= 25
      ) {
...
export function getBatteryIcon(battery) {
  ...
  if (battery.chargePercent > 70)
    return <i class="nf nf-fa-battery_3"></i>;
  if (battery.chargePercent > 40)
